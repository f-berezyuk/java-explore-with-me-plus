...
public interface EndpointHitEntityRepository extends JpaRepository<EndpointHitEntity, Long> {
    ...
    @Query("SELECT new ru.practicum.EWM.stat.dto.ViewStats(s.app, s.uri, COUNT(s.id))" +
    ...
    @Query("SELECT new ru.practicum.EWM.stat.dto.ViewStats(s.app, s.uri, COUNT(distinct s.ip))" +
            "FROM EndpointHitEntity s " +
            "WHERE s.timestamp BETWEEN :start AND :end " +
            "GROUP BY s.app, s.uri " +
